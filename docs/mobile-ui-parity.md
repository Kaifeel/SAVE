...
| Compose | `implemented` | `src/components/ItemRegistrationModal.jsx` | `apps/save-app/src/app/(authenticated)/(tabs)/create.tsx` | Transaction type, title, fee/unit, validated server pickup locations, description, default safety notice, and up to five camera/gallery photos create items through JSON or multipart `POST /items`; pending Android picker-result recovery, validation, permission/settings, empty, error, retry, and authoritative detail-navigation states are covered by direct tests. | Pass: no draft item, pickup location, photo, price, product, or session fixture is used in production; examples appear only as input guidance. | Not run; Android and iPhone camera/gallery permission, multipart upload, keyboard avoidance, and post-create navigation require physical devices. |
...
| My | `implemented` | `src/pages/MyPage.jsx` | `apps/save-app/src/app/(authenticated)/(tabs)/my.tsx`; `apps/save-app/src/app/(authenticated)/notification-settings.tsx` | Authenticated profile, owned items, and wishlist use validated `/users/me`, `/users/me/items`, and `/users/me/wishlist` responses with independent loading, partial-error, empty, retry, stale-request, and pull-to-refresh state. Notification settings use the operating-system permission state and register or unregister the real device token. Real items open detail, the rental action targets `/rentals`, and logout uses secure cleanup with duplicate-tap protection. | Pass: session identity may bridge the profile loading state, but no fabricated profile, item, wishlist, rental, setting, counter, or token is used. | Not run; physical Android/iPhone checks still require devices. Remote push additionally requires linked EAS/FCM credentials. |
| Item detail | `implemented` | `src/ProductDetailPage.jsx` | `apps/save-app/src/app/(authenticated)/(tabs)/items/[id].tsx` | Detail uses `/items/{id}`, wishlist uses `POST`/`DELETE /items/{id}/wishlist`, chat uses the item-backed chat-room API, and rental request uses the rental API after native date/time selection. The screen remains inside the tab navigator, pages all real images with contain sizing, and opens the system share sheet. | Pass: photo counter derives from real images; fixed tags, photo count, trade count, and fallback owner/location values are absent. | Not run; horizontal image paging, sharing, native date/time selection, and Android system-back are not device-verified. |
...
| Rentals | `implemented` | `src/pages/RentalsPage.jsx` | `apps/save-app/src/app/(authenticated)/rentals/index.tsx`; `apps/save-app/src/app/(authenticated)/rentals/[id].tsx` | Item detail creates rental requests with a system date/time picker on Android/iOS and a browser fallback. `/rentals/me` is divided into sent and received requests by authenticated user ID. List and detail support only the backend-authorized `start`, `reject`, `cancel`, and `return` transitions, plus validated review submission, item navigation, chat navigation, retry, pull-to-refresh, confirmation, and notification deep links. | Pass: all rental IDs, roles, status, dates, prices, review state, and navigation targets derive from validated session or server data; no sample rental or unsupported action is rendered. | Not run; native picker behavior, Android notification opening, and iPhone direct route/action checks require physical devices. |
...
3. In Expo LAN development, an empty `EXPO_PUBLIC_API_BASE_URL` is derived from the private Expo development host as `http://<host>:8080/api/v1`.
4. Set `EXPO_PUBLIC_API_BASE_URL` explicitly for an Android emulator (`http://10.0.2.2:8080/api/v1`), Expo tunnel, HTTPS API, nonstandard backend port, and every production build. A physical device must still be on a network that can reach that host and port.
...
The remaining work is device and credential verification rather than another placeholder implementation: camera/gallery multipart upload, native date/time selection, sharing, two-account chat recovery, Google OAuth, and Android push delivery must be rehearsed with the real backend and devices.
...
### Expo native interaction verification from 2026-08-24

| Check | Result |
| --- | --- |
| Expo tests | Pass: 42 suites and 313 tests. |
| Expo TypeScript / ESLint | Pass: `tsc --noEmit` and `expo lint` exited 0. |
| Android export | Pass: Metro bundled 1,252 modules. |
| iOS export | Pass: Metro bundled 1,246 modules. |
| Expo web export | Pass with required runtime environment configured; 38 static routes generated. |
| Expo dependency compatibility | Pass against the SDK 54 local dependency map. The online `expo-doctor` check could not complete in the restricted network. |
| Web regression | Pass: 59 files and 196 tests, ESLint, and production Vite build. |
| Backend regression | Pass: full Gradle test task (`BUILD SUCCESSFUL`). |
| Physical Android/iPhone | Not run: camera, gallery, sharing, date/time picker, and remote API behavior still require a device rehearsal. |
